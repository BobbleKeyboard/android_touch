...
static int handle_mismatch(int fd, struct libevdev *evdev) {
    libevdev_free(evdev);
...
static int start_server(const char *sockname) {
...
int main(int argc, char *argv[]) {
    ...
    char *device = NULL;
    std::string sock_name = "touch";
    const char * sockname = sock_name.c_str();
...
//    }
    ...
    int server_fd = start_server(sock_name.c_str());
